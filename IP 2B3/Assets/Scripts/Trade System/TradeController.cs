...
namespace B3.TradeSystem
{
    public class TradeController : MonoBehaviour
    {
        ...
        public void TradeResources(PlayerBase player, int[] resourcesGiven, int[] resourcesWanted)
        {
            ...
            for (int i = 0; i < resourceTypeCount; i++)
            {
                int want = resourcesWanted[i];
                if (want > 0)
                {
                    var resourceType = (ResourceType)i;

                    // Aflăm câte voia jucătorul și câte are banca înainte de a vinde:
                    int before = bankController.CurrentResources[i];
                    //  ➔ cred că aici a apărut cazul tău cu -1
                    bankController.GetResources(resourceType, want);
                    player.AddResource(resourceType, want);

                    // După ce extragem resursele, logăm noul stoc:
                    Debug.Log($"[TradeResources] Bank after giving {want} of {resourceType}: was {before}, now {bankController.CurrentResources[i]}");
                }
            }
        }
    }
