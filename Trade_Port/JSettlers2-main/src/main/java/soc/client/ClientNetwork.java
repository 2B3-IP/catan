...
/**
 * Helper object to encapsulate and deal with {@link SOCPlayerClient}'s network
 * connectivity.
 * <p>
 * Local practice server (if any) is started in
 * {@link #startPracticeGame(String, SOCGameOptionSet)}.
 * Local tcp server (if any) is started in {@link #initLocalServer(int)}.
 * <br>
 * Messages from server to client are received in either {@link NetReadTask} or
 * {@link LocalStringReaderTask},
 * which call the client's {@link MessageHandler#handle(SOCMessage, boolean)}.
 * Those Task threads call {@link MessageHandler#init(SOCPlayerClient)} before
 * starting their run loop.
 * <br>
 * Messages from client to server are formed in {@link GameMessageSender} or
 * other classes,
 * which call back here to send to the server via {@link #putNet(String)} or
 * {@link #putPractice(String)}.
 * <br>
 * Network shutdown is done in {@link #dispose()}, or {@link #disconnect()} for
 * only the client part.
 * <p>
 * Before v2.0.00, most of these fields and methods were part of the main
 * {@link SOCPlayerClient} class.
 *
...
/* package */ class ClientNetwork {
    ...
    /**
     * When client is running a local server, interval for how often to send
     * {@link SOCServerPing}
     * keepalive messages: 45 minutes, in milliseconds. See
     * {@link #connect(String, int)}.
     * <p>
     * Should be several minutes shorter than
     * {@link soc.server.genericServer.NetConnection#TIMEOUT_VALUE}.
     *
     * @since 2.5.00
     */
    ...
    /**
     * MainDisplay for our {@link #client}, to display information and perform
    ...
    /**
     * Server connection info. {@code null} until {@link #connect(String, int)} is
     * called.
     * Unlike {@code connect} params, localhost is not represented here as
     * {@code null}:
     ...
     * by {@link SOCPlayerClient#shutdownFromNetwork()}; if throwing an error that
     * the user
    ...
    /**
     * Shut down the local TCP server (if any) and {@link #disconnect()} from the
     * network.
     */
    public void dispose() {
        shutdownLocalServer();
     ...
     * @return True if the practice game request was sent, false if there was a
     * problem
    ...
    public boolean startPracticeGame(final String practiceGameName, final SOCGameOptionSet gameOpts) {
        if (practiceServer == null) {
            try {
                ...
                practiceServer = new SOCServer(SOCServer.PRACTICE_STRINGPORT, SOCServer.SOC_MAXCONN_DEFAULT, null,
                        null);
                practiceServer.setPriority(5); // same as in SOCServer.main
                practiceServer.start();
            } catch (Throwable th) {
                mainDisplay.showErrorDialog(client.strings.get("pcli.error.startingpractice") + "\n" + th, // "Problem
                        // starting
                        // practice
                        // server:"
        ...
        if (prCli == null) {
            try {
                ...
                new LocalStringReaderTask(prCli); // practice‐server only
        ...
        if (gameOpts == null)
            ...
            putPractice(SOCNewGameWithOptionsRequest.toCmd(client.practiceNickname, "", SOCMessage.EMPTYSTR,
    ...
    public boolean initLocalServer(int tport) {
        try {
            ...
            mainDisplay.showErrorDialog(client.strings.get("pcli.error.startingserv") + "\n" + th, // "Problem starting
                    // server:"
                    client.strings.get("base.cancel"));
     ...
     * From server to client when client connects: If server is full, it sends
     * {@link SOCRejectConnection}.
     * Otherwise its {@code SOCVersion} and current channels and games
     * ({@link SOCChannels}, {@link SOCGames}) are sent.
     * <p>
     * Since user login and authentication don't occur until a game or channel join
     * is requested,
     * no username or password is needed here.
     * <p>
     * If {@code host} is {@code null}, assumes client has started a local TCP
     * server and is now connecting to it.
     * Will start a thread here to periodically {@link SOCServerPing} that server to
     * prevent timeouts when idle.
     *
     * @param host Server host to connect to, or {@code null} for localhost
     * @param port Server TCP port to connect to; the default server port is
     *             {@link ClientNetwork#SOC_PORT_DEFAULT}.
     * @throws IllegalStateException if already connected
     *                               or if {@link Version#versionNumber()} returns 0
     *                               (packaging error)
    ...
    public synchronized void connect(final String host, final int port)
        ...
        System.out.println(/* I */"Connecting to " + hostString + ":" + port/* 18N */); // I18N: Not localizing console
        // output yet
        ...
        try {
            ...
            ex = e;
            String msg = client.strings.get("pcli.error.couldnotconnect", ex); // "Could not connect to the server: " +
            // ex
    ...
    /**
     * Construct and send a {@link SOCVersion} message during initial connection to
     * a server.
     * Version message includes features and locale in 2.0.00 and later clients;
     * v1.x.xx servers will ignore them.
     * <p>
     * If debug property {@link SOCPlayerClient#PROP_JSETTLERS_DEBUG_CLIENT_FEATURES
     * PROP_JSETTLERS_DEBUG_CLIENT_FEATURES}
     * is set, its value is sent instead of
     * {@link #cliFeats}.{@link SOCFeatureSet#getEncodedList() getEncodedList()}.
     * Then if debug property
     * {@link SOCDisplaylessPlayerClient#PROP_JSETTLERS_DEBUG_CLIENT_GAMEOPT3P
     * PROP_JSETTLERS_DEBUG_CLIENT_GAMEOPT3P}
     * is set, its value is appended to client features as
     * {@code "com.example.js.feat."} + gameopt3p.
     *
     * @param isPractice True if sending to client's practice server with
     *                   {@link #putPractice(String)},
    ...
    private void sendVersion(final boolean isPractice) {
        ...
        final String msg = SOCVersion.toCmd(Version.versionNumber(), Version.version(), Version.buildnum(),
     ...
     * and calls {@link SOCPlayerClient#shutdownFromNetwork()} to show the error
     * message.
     *
     * @param s the message
     ...
     * If we've started a TCP server, tell all players on that server, and shut it
     * down.
     * <p>
     * <em>
     ...
     * </em>
     *
     * @return Can we still start practice games? (No local exception yet in
     * {@link #ex_P})
     * @since 1.1.00
     */
    ...
    static class NetReadTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                if (net.isConnected()) {
                    net.ex = e;
                    System.out.println("could not read from the net: " + net.ex); // I18N: Not localizing console output
                    // yet
                    client.shutdownFromNetwork();
                }
            }
     ...
     * <p>
     * Before v2.0.00 this class was
     * {@code SOCPlayerClient.SOCPlayerLocalStringReader}.
     *
     * @author jdmonin
     * @see NetReadTask
     * @since 1.1.00
    ...
    class LocalStringReaderTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // ONLY loop over practice‐server text here
                ...
                if (locl.isConnected()) {
                    ex_P = e;
                    System.out.println("could not read from practice server: " + ex_P); // I18N: Not localizing console
                    // output yet
                    client.shutdownFromNetwork();
                }
            }
