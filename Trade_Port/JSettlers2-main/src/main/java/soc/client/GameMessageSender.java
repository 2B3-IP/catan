...
/*package*/ class GameMessageSender
{
    ...
    private static final int WHEAT = 2;
    private static final int WOOL = 3;
    private static final int ORE = 4;

    private String formatResourcesTrade(SOCResourceSet give, SOCResourceSet get)
    {
        String[] names = { "wood", "brick", "wheat", "wool", "ore" };
        int[] giveRes = new int[] {
                give.getAmount(WOOD),
                give.getAmount(BRICK),
                give.getAmount(WHEAT),
                give.getAmount(WOOL),
                give.getAmount(ORE)
        };
        int[] getRes = new int[] {
        ...
        for (int i = 0; i < 5; i++) {
            if (giveRes[i] > 0) {
                if (sbGive.length() > 0) sbGive.append(", ");
                sbGive.append(giveRes[i]).append(" ").append(names[i]);
            }
        }
        ...
        for (int i = 0; i < 5; i++) {
            if (getRes[i] > 0) {
                if (sbGet.length() > 0) sbGet.append(", ");
                sbGet.append(getRes[i]).append(" ").append(names[i]);
            }
        }

        return "traded " + sbGive + " for " + sbGet;
    }

    public void bankTrade(SOCGame ga, SOCResourceSet give, SOCResourceSet get)
    {
        int currentPlayer = ga.getCurrentPlayerNumber();
        String playerName = (currentPlayer == 0) ? "CLIENT" : "BOT" + currentPlayer;

        // Log in consolă
        System.out.println(playerName + " did TRADE_PORT: " + formatResourcesTrade(give, get));

        // Trimite comanda de trade
